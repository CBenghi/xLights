...
#include "../../include/Rainbow-16.xpm"
#include "../../include/Rainbow-64.xpm"

#include "RainbowEffect.h"
#include "RainbowPanel.h"
...
public:
    ...
    RainbowTiming(uint32_t start, uint32_t end)
    {
        _startMS = start;
        _endMS = end;
    }
...
public:
    ...
    std::list<std::pair<uint8_t, uint8_t>> _RainbowTiming;
};

std::list<Chordx> __RainbowChords = {
    // Major Chords
...
bool centresort(const RainbowTiming* first, const RainbowTiming* second)
{
...
public:
    ...
    uint32_t _endMS = 0;
    std::list<uint8_t> _notes;
    std::list<RainbowTiming*> _possibleTimings;
    ...
    NoteTiming(const NoteTiming& nt)
    {
        ...
        for (auto it : nt._possibleTimings) {
            _possibleTimings.push_back(new RainbowTiming(*it));
        }
    }

    void SetRainbowTiming(RainbowTiming* gt)
    {
        ClearPossibleTimings();
        _possibleTimings.push_back(new RainbowTiming(*gt));
    }

    static const std::vector<RainbowNotes>* GetBaseNotes(const std::string& type)
    {
        if (type == "Rainbow")
            return &Rainbow;
        else if (type == "Bass Rainbow")
            return &bass;
        else if (type == "Banjo")
            return &banjo;
        else
        ...
        for (auto& it : chords) {
            if (it._notes.size() == _notes.size()) {
                if (listsMatch(it._notes, _notes)) {
                    auto t = new RainbowTiming(_startMS, _endMS);
                    for (auto const& it2 : it._RainbowTiming) {
...
#endif
                auto fns = j;
                RainbowTiming* t = new RainbowTiming(_startMS, _endMS);
                for (auto n : _notes) {
#ifdef FILTER_OUT_UPLAYABLE
                    bool found = false;
#endif
...
RainbowEffect::RainbowEffect(int id) :
    RenderableEffect(id, "Rainbow", Rainbow_16_xpm, Rainbow_64_xpm, Rainbow_64_xpm, Rainbow_64_xpm, Rainbow_64_xpm) 
{
    //ctor
	_panel = nullptr;
}
...
std::list<std::string> RainbowEffect::CheckEffectSettings(const SettingsMap& settings, AudioManager* media, Model* model, Effect* eff, bool renderCache)
{
    std::list<std::string> res;

    if (settings.Get("E_CHOICE_Rainbow_MIDITrack_APPLYLAST", "") == "")
    {
        res.push_back(wxString::Format("    ERR: Rainbow effect needs a timing track. Model '%s', Start %s", model->GetName(), FORMATTIME(eff->GetStartTimeMS())).ToStdString());
    }
    else
    {
        std::list<NoteTiming*> timings = LoadTimingTrack(settings.Get("E_CHOICE_Rainbow_MIDITrack_APPLYLAST", ""), 50, "Rainbow", 100, 6);
        if (timings.size() == 0)
        {
            res.push_back(wxString::Format("    ERR: Rainbow effect timing track '%s' has no notes. Model '%s', Start %s", settings.Get("E_CHOICE_Rainbow_MIDITrack_APPLYLAST", ""), model->GetName(), FORMATTIME(eff->GetStartTimeMS())).ToStdString());
        }
...
void RainbowEffect::SetPanelStatus(Model *cls)
{
    SetPanelTimingTracks();
}

void RainbowEffect::SetPanelTimingTracks()
{
    RainbowPanel *fp = (RainbowPanel*)panel;
    if (fp == nullptr)
    {
        return;
    }
...
xlEffectPanel *RainbowEffect::CreatePanel(wxWindow *parent) {
    _panel = new RainbowPanel(parent);
	return _panel;
}

void RainbowEffect::SetDefaultParameters() {
    RainbowPanel *pp = (RainbowPanel*)panel;
    if (pp == nullptr) {
        return;
    }

    SetChoiceValue(pp->Choice_Rainbow_Type, "Rainbow");
...
void RainbowEffect::RenameTimingTrack(std::string oldname, std::string newname, Effect* effect)
{
    wxString timing = effect->GetSettings().Get("E_CHOICE_Rainbow_MIDITrack_APPLYLAST", "");

    if (timing.ToStdString() == oldname)
    {
        effect->GetSettings()["E_CHOICE_Rainbow_MIDITrack_APPLYLAST"] = wxString(newname);
    }

    SetPanelTimingTracks();
}

void RainbowEffect::Render(Effect *effect, const SettingsMap &SettingsMap, RenderBuffer &buffer) {
    RenderRainbow(buffer,
                 effect->GetParentEffectLayer()->GetParentElement()->GetSequenceElements(),
                 std::string(SettingsMap.Get("CHOICE_Rainbow_Type", "Rainbow")),
                 std::string(SettingsMap.Get("CHOICE_Rainbow_MIDITrack_APPLYLAST", "")),
...
class RainbowCache : public EffectRenderCache
{
public:
	RainbowCache() { };
	virtual ~RainbowCache() { 
        ClearTimings();
    };
    ...
    void SetTimings(std::list<NoteTiming*>& notes, const std::string& type, uint8_t maxFrets)
    {
        ...
        ConvertToRainbowTiming(type, maxFrets);
    }

    void ConvertToRainbowTiming(const std::string& type, uint8_t maxFrets)
    {
        // sort the notes
    ...
    RainbowTiming* GetTimingAt(uint32_t ms)
    {
...
//render Rainbow fx during sequence:
void RainbowEffect::RenderRainbow(RenderBuffer& buffer, SequenceElements* elements, const std::string& type, const std::string& MIDITrack, const std::string& stringAppearance, int maxFrets, bool showStrings, bool fade, bool collapse, double stringWaveFactor, double baseWaveFactor, bool varyWavelengthBasedOnFret)
{
    RainbowCache* cache = (RainbowCache*)buffer.infoCache[id];
    if (cache == nullptr) {
        cache = new RainbowCache();
    ...
    DrawRainbow(buffer, cache->GetTimingAt(time), stringAppearance, maxFrets, strings, showStrings, fade, collapse, stringWaveFactor, baseWaveFactor, varyWavelengthBasedOnFret);
}
...
void RainbowEffect::DrawRainbowOn(RenderBuffer& buffer, uint8_t string, uint8_t fretPos, uint32_t timePos, uint32_t of, uint8_t maxFrets, uint8_t strings, bool showStrings, bool fade, bool collapse)
{
    xlColor c;
...
void RainbowEffect::DrawRainbowWave(RenderBuffer& buffer, uint8_t string, uint8_t fretPos, uint32_t timePos, uint32_t of, uint8_t maxFrets, uint8_t strings, bool showStrings, bool fade, bool collapse, double stringWaveFactor, double baseWaveFactor, bool varyWavelengthBasedOnFret)
{
    xlColor c;
    buffer.palette.GetColor(string % buffer.palette.Size(), c);
...
void RainbowEffect::DrawString(RenderBuffer& buffer, uint8_t string, uint8_t strings)
{
    xlColor c;
...
void RainbowEffect::DrawRainbow(RenderBuffer& buffer, RainbowTiming* pdata, const std::string& stringAppearance, uint8_t maxFrets, uint8_t strings, bool showStrings, bool fade, bool collapse, double stringWaveFactor, double baseWaveFactor, bool varyWavelengthBasedOnFret)
{
    if (pdata != nullptr) {
        ...
        if (stringAppearance == "On") {
            for (const auto& it : pdata->_fingerPos) {
                DrawRainbowOn(buffer, it.first, it.second, pos, len, maxFrets, strings, showStrings, fade, collapse);
            }
        } else if (stringAppearance == "Wave") {
            for (const auto& it : pdata->_fingerPos) {
                DrawRainbowWave(buffer, it.first, it.second, pos, len, maxFrets, strings, showStrings, fade, collapse, stringWaveFactor, baseWaveFactor, varyWavelengthBasedOnFret);
            }
        }
    }
...
std::vector<float> RainbowEffect::Parse(wxString& l)
{
...
std::list<std::string> RainbowEffect::ExtractNotes(const std::string& label)
{
    std::string n = label;
...
void RainbowEffect::ConvertStringPos(const std::string& note, uint8_t& string, uint8_t& pos)
{
    string = 0xFF;
    pos = 0xFF;
...
int RainbowEffect::ConvertNote(const std::string& note)
{
    std::string n = note;
    int nletter;
    std::transform(n.begin(), n.end(), n.begin(), ::toupper);
...
std::list<NoteTiming*> RainbowEffect::LoadTimingTrack(const std::string& track, int intervalMS, const std::string& type, uint8_t maxFrets, uint8_t strings)
{
    static log4cpp::Category& logger_Rainbowdata = log4cpp::Category::getInstance(std::string("log_Rainbowdata"));

    std::list<NoteTiming*> res;

    logger_Rainbowdata.debug("Loading timings from timing track " + track);

    if (mSequenceElements == nullptr) {
        logger_Rainbowdata.debug("No timing tracks found.");
        return res;
    }
    ...
    if (el == nullptr) {
        logger_Rainbowdata.debug("Timing track not found.");
        return res;
    }
    ...
    for (int j = 0; j < el->GetEffectCount(); ++j) {
        ...
        RainbowTiming t(starttime, endtime);
