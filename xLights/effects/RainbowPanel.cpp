...
//(*IdInit(RainbowPanel)
const long RainbowPanel::ID_STATICTEXT_Rainbow_Type = wxNewId();
const long RainbowPanel::ID_CHOICE_Rainbow_Type = wxNewId();
const long RainbowPanel::ID_STATICTEXT_Rainbow_MIDITrack_APPLYLAST = wxNewId();
const long RainbowPanel::ID_CHOICE_Rainbow_MIDITrack_APPLYLAST = wxNewId();
const long RainbowPanel::ID_STATICTEXT1 = wxNewId();
const long RainbowPanel::ID_CHOICE_StringAppearance = wxNewId();
const long RainbowPanel::ID_STATICTEXT_Piano_Scale = wxNewId();
const long RainbowPanel::ID_SLIDER_MaxFrets = wxNewId();
const long RainbowPanel::IDD_TEXTCTRL_MaxFrets = wxNewId();
const long RainbowPanel::ID_STATICTEXT2 = wxNewId();
const long RainbowPanel::ID_SLIDER_BaseWaveFactor = wxNewId();
const long RainbowPanel::IDD_TEXTCTRL_BaseWaveFactor = wxNewId();
const long RainbowPanel::ID_STATICTEXT3 = wxNewId();
const long RainbowPanel::ID_SLIDER_StringWaveFactor = wxNewId();
const long RainbowPanel::IDD_TEXTCTRL_StringWaveFactor = wxNewId();
const long RainbowPanel::ID_CHECKBOX_Fade = wxNewId();
const long RainbowPanel::ID_CHECKBOX_Collapse = wxNewId();
const long RainbowPanel::ID_CHECKBOX_ShowStrings = wxNewId();
const long RainbowPanel::ID_CHECKBOX_VaryWaveLengthOnFret = wxNewId();
//*)

BEGIN_EVENT_TABLE(RainbowPanel,wxPanel)
	//(*EventTable(RainbowPanel)
	//*)
    EVT_COMMAND(wxID_ANY, EVT_SETTIMINGTRACKS, RainbowPanel::SetTimingTracks)
END_EVENT_TABLE()

RainbowPanel::RainbowPanel(wxWindow* parent) : xlEffectPanel(parent)
{
	//(*Initialize(RainbowPanel)
	...
	StaticText1 = new wxStaticText(this, ID_STATICTEXT_Rainbow_Type, _("Type"), wxDefaultPosition, wxDefaultSize, 0, _T("ID_STATICTEXT_Rainbow_Type"));
	FlexGridSizer27->Add(StaticText1, 1, wxALL|wxALIGN_LEFT|wxALIGN_CENTER_VERTICAL, 2);
	Choice_Rainbow_Type = new BulkEditChoice(this, ID_CHOICE_Rainbow_Type, wxDefaultPosition, wxDefaultSize, 0, 0, 0, wxDefaultValidator, _T("ID_CHOICE_Rainbow_Type"));
	Choice_Rainbow_Type->SetSelection( Choice_Rainbow_Type->Append(_("Rainbow")) );
	Choice_Rainbow_Type->Append(_("Bass Rainbow"));
	Choice_Rainbow_Type->Append(_("Banjo"));
	FlexGridSizer27->Add(Choice_Rainbow_Type, 1, wxALL|wxEXPAND, 2);
	FlexGridSizer27->Add(-1,-1,1, wxALL|wxEXPAND, 5);
	StaticText8 = new wxStaticText(this, ID_STATICTEXT_Rainbow_MIDITrack_APPLYLAST, _("Track"), wxDefaultPosition, wxDefaultSize, 0, _T("ID_STATICTEXT_Rainbow_MIDITrack_APPLYLAST"));
	FlexGridSizer27->Add(StaticText8, 1, wxALL|wxALIGN_LEFT|wxALIGN_CENTER_VERTICAL, 2);
	Choice_Rainbow_MIDITrack_APPLYLAST = new BulkEditChoice(this, ID_CHOICE_Rainbow_MIDITrack_APPLYLAST, wxDefaultPosition, wxDefaultSize, 0, 0, 0, wxDefaultValidator, _T("ID_CHOICE_Rainbow_MIDITrack_APPLYLAST"));
	FlexGridSizer27->Add(Choice_Rainbow_MIDITrack_APPLYLAST, 1, wxALL|wxEXPAND, 2);
	FlexGridSizer27->Add(-1,-1,1, wxALL|wxEXPAND, 5);
    ...
    SetName("ID_PANEL_Rainbow");

	Choice_Rainbow_Type->Append(_("Violin"));

    Connect(wxID_ANY, EVT_VC_CHANGED, (wxObjectEventFunction)&RainbowPanel::OnVCChanged, nullptr, this);
    Connect(wxID_ANY, EVT_VALIDATEWINDOW, (wxObjectEventFunction)&RainbowPanel::OnValidateWindow, 0, this);

	ValidateWindow();
}

RainbowPanel::~RainbowPanel()
{
	//(*Destroy(RainbowPanel)
	//*)
}

void RainbowPanel::ValidateWindow()
{
    if (Choice_Rainbow_MIDITrack_APPLYLAST->GetCount() > 0) {
        Choice_Rainbow_MIDITrack_APPLYLAST->Enable(true);
    } else {
        Choice_Rainbow_MIDITrack_APPLYLAST->Enable(false);
    }
...
void RainbowPanel::SetTimingTracks(wxCommandEvent& event)
{
    auto timingtracks = wxSplit(event.GetString(), '|');

    wxString selection = Choice_Rainbow_MIDITrack_APPLYLAST->GetStringSelection();
    ...
    bool removed = false;
    for (size_t i = 0; i < Choice_Rainbow_MIDITrack_APPLYLAST->GetCount(); i++)
    {
        bool found = false;
        for (auto it = timingtracks.begin(); it != timingtracks.end(); ++it)
        {
            if (*it == Choice_Rainbow_MIDITrack_APPLYLAST->GetString(i))
            {
                found = true;
                break;
            }
        ...
        if (!found)
        {
            Choice_Rainbow_MIDITrack_APPLYLAST->Clear();
            removed = true;
            break;
        }
    }
    ...
    for (auto it = timingtracks.begin(); it != timingtracks.end(); ++it)
    {
        ...
        if (!found)
        {
            Choice_Rainbow_MIDITrack_APPLYLAST->Append(*it);
        }
    }

    if (removed && Choice_Rainbow_MIDITrack_APPLYLAST->GetCount() > 0)
    {
        // go through the list and see if our selected item is there
        bool found = false;
        for (size_t i = 0; i < Choice_Rainbow_MIDITrack_APPLYLAST->GetCount(); i++)
        {
            if (selection == Choice_Rainbow_MIDITrack_APPLYLAST->GetString(i))
            {
                found = true;
                Choice_Rainbow_MIDITrack_APPLYLAST->SetSelection(i);
                break;
            }
        }
        if (!found)
        {
            Choice_Rainbow_MIDITrack_APPLYLAST->SetSelection(0);
        }
    }
    ValidateWindow();
}

void RainbowPanel::OnChoice_StringAppearanceSelect(wxCommandEvent& event)
{
    ValidateWindow();
}
